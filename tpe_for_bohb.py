...
class TPE_Advisor:
    ...
    def get_suggestions(self, n, history=None):
        if history is None:
        ...
        if num_config_evaluated == 0:
            return self.sample_random_configs(n, history)
        ...
        try:
            ...
            if best_vector is None:
                ...
                configs = self.sample_random_configs(n, history)
            else:
                logger.debug(
                    'best_vector: {}, {}, {}, {}'.format(best_vector, best, l(best_vector), g(best_vector)))

                sorted_idx = np.argsort(val_list)
                sorted_vector_list = np.array(vector_list)[sorted_idx]
                for vector in sorted_vector_list:
                    ...
                    try:
                        ...
                        logger.warning(
                            "Converting configuration error. Using random configuration")
